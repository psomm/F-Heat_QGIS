...
class HeatNetTool:
    ...
    def install_package(self):
        '''Installiert Pakete in eine isolierte virtuelle Umgebung.'''
        ...
        from .src.core.venv_manager import packages_installed
        if packages_installed():
            QMessageBox.information(
                self.dlg,
                self.tr('Packages already installed'),
                self.tr('All required packages are already installed in the virtual environment. No action needed.')
            )
            self.dlg.intro_label.setText(self.tr('✓ Alle Pakete bereits installiert.'))
            self.dlg.intro_label.setStyleSheet('color: #55ff55')
            return

        from qgis.PyQt.QtWidgets import QProgressBar

        # Store as instance variable so it is not garbage-collected while the
        # Worker thread still holds a reference to gui_elements['progressBar'].
        self._install_dummy_bar = QProgressBar(self.dlg)

        gui_elements = {
            'progressBar': self._install_dummy_bar,
            'label': self.dlg.intro_label
        }

        def install_task(progress_signal, label_signal):
            # Import inside the worker thread so any import error surfaces via error_occurred
            from .src.core.venv_manager import create_venv_and_install
            def progress_callback(pct, msg):
                progress_signal.emit(pct)
                label_signal.emit(msg, 'orange')
            ok, msg = create_venv_and_install(progress_callback=progress_callback)
            if not ok:
                raise RuntimeError(msg)

        def on_finished():
            self.worker_running = False
            self.dlg.intro_label.setText(self.tr('✓ Fertig! Bitte QGIS neu starten.'))
            self.dlg.intro_label.setStyleSheet('color: #55ff55')

        self.worker_running = True
        self.run_long_task(install_task, gui_elements, on_finished)
    ...
    def run(self):
        ...
        # venv-Pakete zu sys.path hinzufügen (falls venv bereits installiert)
        try:
            from .src.core.venv_manager import ensure_packages_available
